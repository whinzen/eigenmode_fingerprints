...
def main():
    ...
    mode3_L = zscore(L[:, 3])
    mode3_R = zscore(-R[:, 3])  # same RH sign convention as your figures

    # fetch canonical Gradient 1
    ...
    # resample to fsaverage 10k ~= fsaverage5 resolution
    ...
    print(f"Left : r = {rL:.4f}, |r| = {abs(rL):.4f}, p = {pL:.3e}")
    print(f"Right: r = {rR:.4f}, |r| = {abs(rR):.4f}, p = {pR:.3e}")
