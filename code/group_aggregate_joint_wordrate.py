...
OUT_DIR = BASE / "group_joint_wordrate"
...
def aggregate_one(metric):
    ...
    for sub_dir in sorted(BASE.glob("sub-*")):
        f = sub_dir / f"glm_joint_wordrate_{metric}" / f"joint_wordrate_{metric}_wide.csv"
        ...
        required = {"subject", "run", "hemi", "mode_k", "beta_wordrate", "beta_metric"}
        if not required.issubset(df.columns):
            print(f"[bad columns skipped] {f} → {list(df.columns)}")
            continue

        rows.append(df)

    if not rows:
        raise RuntimeError(f"No usable joint wordrate files found for {metric}")

    big = pd.concat(rows, ignore_index=True)
    print(f"Collected rows: {len(big)}")

    big.to_csv(OUT_DIR / f"group_joint_wordrate_{metric}_allruns.csv", index=False)
    ...
    for hemi, d_hemi in big.groupby("hemi"):
        ...
        for mode_k, g in subj_mean.groupby("mode_k"):
            ...
            out_rows.append({
                ...
                "p_wordrate": p_wr,
                "beta_metric_mean": mean_met,
                "beta_metric_sem": sem_met,
                "p_metric": p_met,
                "N_subj": len(g),
            })
    ...
    for hemi in out["hemi"].unique():
        ...
        out.loc[mask, "sig_metric_q05"] = bh_fdr(out.loc[mask, "p_metric"].values)

    out["sig_wordrate_q05"] = out["sig_wordrate_q05"].astype(int)
    out["sig_metric_q05"] = out["sig_metric_q05"].astype(int)

    out_csv = OUT_DIR / f"group_joint_wordrate_{metric}_by_mode_subject_level.csv"
