...
OUT_DIR = BASE / "group_sentence_level_glm"
...
def load_lambda_table():
    ...
    if "mode_k" not in t.columns or "lam" not in t.columns:
        raise RuntimeError(f"wavelength table missing required columns: {list(t.columns)}")
    return t[["mode_k", "lam"]].drop_duplicates("mode_k")


def collect_subject_wide(metric):
    rows = []
    skipped_missing = 0
    skipped_empty = 0
    ...
    for sub in sorted(BASE.glob("sub-*")):
        ...
        if metric == "boundary":
            f = sub / "glm_boundary" / "boundary_wide.csv"
        elif metric == "sentence_shift":
            f = sub / "glm_sentence_shift" / "sentence_shift_wide.csv"
        else:
    ...
    print(f"[{metric}] loaded subjects: {len(rows)} | missing: {skipped_missing} | empty: {skipped_empty}")

    if not rows:
        raise ValueError(f"No non-empty files found for metric={metric}")
...
def aggregate_metric(metric):
    ...
    big = collect_subject_wide(metric)

    out_big = OUT_DIR / f"group_{metric}_allruns.csv"
    ...
    for hemi in ["L", "R"]:
        mask = out["hemi"] == hemi
        out.loc[mask, "sig_q05"] = bh_fdr(out.loc[mask, "p"].values, alpha=0.05).astype(int)
