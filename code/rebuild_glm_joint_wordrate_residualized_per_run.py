...
def residualize_against(x, base):
    """
    Residualize x with respect to base:
        x_res = x - P_base x
    """
    x = np.asarray(x, float)
    base = np.asarray(base, float)

    good = np.isfinite(x) & np.isfinite(base)
    xg = x[good]
    bg = base[good]
    ...
    B = np.column_stack([np.ones(len(bg)), bg])
    beta, _, _, _ = np.linalg.lstsq(B, xg, rcond=None)
    xhat = B @ beta
    resid = xg - xhat
...
def fit_ols_multi(y, X):
    ...
    OLS:
        y = b0 + X b + e
    Returns betas for predictors only (excluding intercept), and p-values.
    """
...
def main():
    ...
    for metric_name, metric_dir in METRIC_DIRS.items():
        print(f"\n=== RESIDUALIZED JOINT GLM: wordrate + {metric_name}_res ===")
        ...
        for sub_dir in subjects:
            ...
            out_dir = sub_dir / f"glm_joint_wordrate_resid_{metric_name}"
            ...
            for run in runs:
                ...
                # residualize metric against wordrate, then z-score residual
                x_met_res = residualize_against(x_met, x_wr)
                x_met_res = zscore_safe(x_met_res[np.isfinite(x_met_res)]) if np.isfinite(x_met_res).any() else x_met_res

                # restore shape after zscoring finite values only
                if np.ndim(x_met_res) == 1 and len(x_met_res) != len(x_wr):
                    tmp = residualize_against(x_met, x_wr)
                    good = np.isfinite(tmp)
                    tmp2 = tmp.copy()
                    tmp2[good] = zscore_safe(tmp[good])
                    x_met_res = tmp2
                ...
                for hemi in HEMIS:
                    ...
                    aligned = align_all([E, x_wr, x_met_res])
                    if aligned is None:
                        continue
                    ...
                    X = np.column_stack([wr_al, met_al])
                    ...
                    per_run_df = pd.DataFrame({
                        ...
                        "beta_metric_resid": betas_met,
                        "p_wordrate": p_wr,
                        "p_metric_resid": p_met,
                        "metric": metric_name,
                    })

                    out_csv = out_dir / f"joint_wordrate_resid_{metric_name}_{hemi}_run-{run:02d}.csv"
                    per_run_df.to_csv(out_csv, index=False)

            if len(wide_rows) == 0:
                print(f"[skip write empty] {sub} {metric_name}")
                continue

            wide_df = pd.DataFrame(wide_rows)
            wide_out = out_dir / f"joint_wordrate_resid_{metric_name}_wide.csv"
