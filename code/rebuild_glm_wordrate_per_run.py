...
BASE = Path(PANG_OUT)
REG_DIR = BASE / "regressors" / "wordrate_per_subject"
    ...
    for sub_dir in subjects:
        ...
        for run in runs:
            ...
            if not reg_path.exists():
                print(f"[skip] {sub} run-{run:02d}: missing wordrate regressor")
                continue
            ...
            for hemi in HEMIS:
                ...
                out_csv = out_dir / f"wordrate_{hemi}_run-{run:02d}.csv"
                per_run_df.to_csv(out_csv, index=False)

        wide_df = pd.DataFrame(wide_rows)
        wide_out = out_dir / "wordrate_wide.csv"
